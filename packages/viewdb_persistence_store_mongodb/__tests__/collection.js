...
describe('mongodb_persistence', () => {
  ...
  describe('Collection', function () {
    ...
    it('#insert two documents with same key should throw', () =>
      new Promise((resolve, reject) => {
        ...
        store.open().then(function () {
          store
            .collection(COLLECTION_NAME)
            .insert({ _id: 'echo' }, function () {
              store
                .collection(COLLECTION_NAME)
                .insert({ _id: 'echo' }, function (err) {
                  if (err) {
                    resolve();
                  } else {
    ...
    it('#findAndModify upsert', async function () {
      ...
      await store
        .collection(COLLECTION_NAME)
        .findAndModify(filter, sort, update, options);
    ...
    it('#findAndModify modify', async function () {
      ...
      const options = { upsert: true };
      await store.open();
      await store.collection(COLLECTION_NAME).save({
        _id: 'not-existing',
        test: 2,
      ...
      new Promise((resolve, reject) => {
        ...
        store.open().then(function () {
          store
            .collection(COLLECTION_NAME)
            .insert(
              { _id: 'echo', name: { first: 'ECHO', last: 'TV' } },
              function () {
                store
                  .collection(COLLECTION_NAME)
                  .insert(
                    { _id: 'sierra', name: { first: 'SIERRA', last: 'TV' } },
                    function () {
                      store
                        .collection(COLLECTION_NAME)
