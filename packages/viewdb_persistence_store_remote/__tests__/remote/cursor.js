...
describe('Remote server/client', function () {
  ...
  it('#remote collection count', () =>
    new Promise((resolve, reject) => {
      ...
      clientVdb
        .collection('dollhouse')
        .count({ _id: 'echo' }, function (err, res) {
          res.should.equal(1);
          resolve();
        });
    }));
  ...
  it('#remote collection count with limit', () =>
    new Promise((resolve, reject) => {
      ...
      clientVdb
        .collection('dollhouse')
        .count({}, { limit: 1 }, function (err, res) {
          res.should.equal(1);
          resolve();
        });
    }));
  it('#remote collection observe', () =>
    new Promise((resolve, reject) => {
      remote.collection('dollhouse').insert({ _id: 'echo' });
      remote.collection('dollhouse').insert({ _id: 'echo2' });
      var cursor = clientVdb
        .collection('dollhouse')
        .find({ _id: { $in: ['echo2', 'echo3'] } });
      cursor.observe({
        init: function (init) {
  ...
  it('#remote reconnected should work with hybrid', () =>
    new Promise((resolve, reject) => {
      ...
      var hybrid = new ViewDb(
        new HybridStore(local, clientStore, { throttleObserveRefresh: 0 }),
      );
      var list = [];
      var changes = 0;
